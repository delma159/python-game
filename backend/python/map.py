    ...
    def generate_map(self):
        ...
        if self.longeur < 4 or self.longeur > 11:
            ...
            for colonne in range(self.largeur):
                ...
                for ligne in self.listeMap:
                    for cell in range(self.longeur):
                        self.listeMap[cpt].append(CaseType("vide", 1, colonne, cell))

                cpt += 1



            self.listeMap[0][1] = CaseType("princess", 3, 0, 1)
            self.listeMap[-1][1] = CaseType("perso", 4, -1, 1)


         
            compteur=0
            for e in self.listeMap:
                self.listeMap[compteur].append(CaseType("mur", 2, compteur,0))
                random.shuffle(self.listeMap[compteur])
                compteur += 1



        return print(self.listeMap)
